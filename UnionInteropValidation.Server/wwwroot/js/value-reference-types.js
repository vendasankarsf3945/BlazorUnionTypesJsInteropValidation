...
export async function callCSharpWithValueType() {
    ...
    try {
        const result = await DotNet.invokeMethodAsync("UnionInteropValidation.Server", "HandleValueTypeFromJS", { value: 777 });
...
export async function callCSharpWithReferenceType() {
    ...
    try {
        const result = await DotNet.invokeMethodAsync("UnionInteropValidation.Server", "HandleReferenceTypeFromJS", { content: "from JS", metadata: "test" });
        console.log("C# response:", result);
        return result;
    } catch (error) {
        console.error("Error calling C# method:", error);
