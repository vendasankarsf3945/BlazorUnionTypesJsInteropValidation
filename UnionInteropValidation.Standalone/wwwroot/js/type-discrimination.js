...
export async function callCSharpWithPropertyBased() {
    ...
    try {
        const result = await DotNet.invokeMethodAsync("UnionInteropSample", "HandlePropertyBasedFromJS", { result: "success from JS" });
...
export async function callCSharpWithTypeBased() {
    ...
    try {
        const result = await DotNet.invokeMethodAsync("UnionInteropSample", "HandleTypeBasedFromJS", { result: "success from JS" });
        console.log("C# response:", result);
        return result;
    } catch (error) {
        console.error("Error calling C# method:", error);
